    ...
    def __what_to_sell(self, current_date, signals):
        ...
        for trading_pair, value in self.trading_pairs.items():
            ...
            if self.is_fake_api():
                ...
                if (crypto_amount > 0) and (len(self.y_buy[value.name]) > 0):
                    order_price = self.y_buy[value.name][-1]
                    current_price = self.trading_api.get_price_ticker(value.base_asset, value.quote_asset, current_date)
